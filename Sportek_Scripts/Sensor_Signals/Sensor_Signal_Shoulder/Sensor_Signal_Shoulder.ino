...
#include <Wire.h>
//y axis
#define INTEREST_AXES 1

BLEService sensorService("19B10016-E8F2-537E-4F6C-D104768A1214");
BLEIntCharacteristic interestAcceleration("19B10011-E8F2-537E-4F6C-D104768A1214", BLERead | BLENotify);
...
void setup() {
  ...
  BLE.setLocalName("Shoulder BLE");
  BLE.setAdvertisedService(sensorService);
  sensorService.addCharacteristic(interestAcceleration);
  sensorService.addCharacteristic(interestAngle);
  BLE.addService(sensorService);

  interestAcceleration.writeValue(0);
...
void loop() {
  ...
  if (central){
    ...
    while(central.connected()){
      ...
      interestAcceleration.writeValue((int)accVals[INTEREST_AXES]);
      interestAngles.writeValue((int)netAngles[INTEREST_AXES]);
